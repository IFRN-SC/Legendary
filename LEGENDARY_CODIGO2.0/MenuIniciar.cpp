...
void MenuIniciar::menuSerial(){
  ...
  while(sairMenuSerial == false){
    if(naoImprimir == false){
      ...
      Serial.println(F("  9- Testar Alavancas"));
      Serial.println(F("  10- Ver valores das Calibragens"));
      Serial.println(F("  11- Limpar Roda"));
    ...
    switch(i){
      ...
      case 8: //Calibrar Sensor IMU
        calibragem.calibrarIMU();
        naoImprimir = false;
        break;
        
      case 9:
        Serial.println(F("\nEntrando em Loop para Testar as Alavancas..."));
        Serial.println(F("(Digite -1 para sair da Repetição)\n"));
        while(Serial.parseInt() != -1){
          alavanca.testarSensoresAlavanca();
          delay(300);
        }
        naoImprimir = false;
        break;
        
      case 10: //Ver dados Calibração
        Serial.println(F(""));
